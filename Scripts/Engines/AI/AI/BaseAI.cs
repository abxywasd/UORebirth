...
namespace Server.Mobiles
{
	...
	public abstract class BaseAI
	{
		...
		private class InternalEntry : ContextMenuEntry
		{
			...
			{
				if ( !m_Mobile.Deleted && m_Mobile.Controled && m_From == m_Mobile.ControlMaster && m_From.CheckAlive() )
				{
					switch ( m_Order )
					{
        ...
        public virtual void GetContextMenuEntries(Mobile from, List<ContextMenus.ContextMenuEntry> list)
		{
			if ( from.Alive && m_Mobile.Controled && from == m_Mobile.ControlMaster && from.InRange( m_Mobile, 14 ) )
			{
		...
		{
			if ( m_Mobile.Deleted || !m_Mobile.Controled || from != m_Mobile.ControlMaster || !from.InRange( m_Mobile, 14 ) || from.Map != m_Mobile.Map )
				return;
		...
		{
			if ( m_Mobile.Deleted || !m_Mobile.Controled || from != m_Mobile.ControlMaster || !from.InRange( m_Mobile, 14 ) || from.Map != m_Mobile.Map || !from.CheckAlive() )
				return;

			if ( order == OrderType.Attack && target is BaseCreature && ((BaseCreature)target).IsScaryToPets && m_Mobile.IsScaredOfScaryThings )
			{
		...
		public virtual bool HandlesOnSpeech( Mobile from )
		{
			...
			if ( from.Alive && m_Mobile.Controled && m_Mobile.Commandable && from == m_Mobile.ControlMaster )
				return true;
		...
		public virtual bool DoActionCombat()
		{
			...
			if ( c == null || c.Deleted || c.Map != m_Mobile.Map || !m_Mobile.InRange( c.Location, (int)(m_Mobile.RangePerception*1.5) ) || !c.Alive || c.IsDeadBondedPet )
				Action = ActionType.Wander;
			else
		...
		public virtual bool DoActionFlee()
		{
			...
			if ( WalkMobileRange( from, 1, false, m_Mobile.RangePerception, (int)(m_Mobile.RangePerception*1.5) ) )
			{
		...
		private int m_NumCommands;

		public int NumCommands { get { return m_NumCommands; } set { m_NumCommands = value; } }
		...
		public virtual void OnCurrentOrderChanged()
		{
			...
			if ( !m_Mobile.Body.IsHuman && m_Mobile.Controled && !m_Mobile.Summoned && m_Mobile.ControlOrder != OrderType.None )
			{
				m_NumCommands++;
				if ( m_NumCommands >= 200 )
				{
					m_NumCommands = 0;
					m_Mobile.Loyalty--;
					
					if ( m_Mobile.Loyalty == PetLoyalty.None )
					{
						if ( m_Mobile is BaseMount )
							((BaseMount)m_Mobile).Rider = null;
						m_Mobile.Loyalty = PetLoyalty.WonderfullyHappy;
					...
					{
						if ( m_Mobile.Loyalty == PetLoyalty.Confused )
							m_Mobile.Say( 1043270, m_Mobile.Name ); // * ~1_NAME~ looks around desperately *
		...
		public virtual bool DoOrderCome()
		{
			if ( m_Mobile.ControlMaster != null && !m_Mobile.ControlMaster.Deleted )
			{
				...
				else
				{
					...
					if ( WalkMobileRange( m_Mobile.ControlMaster, 1, true, 0, 1 ) )
					{
						if ( m_Mobile.Combatant != null && !m_Mobile.Combatant.Deleted && m_Mobile.Combatant.Alive && !m_Mobile.Combatant.IsDeadBondedPet )
						{
							m_Mobile.Warmode = true;
		...
		public virtual bool DoOrderFollow()
		{
			...
			else if ( m_Mobile.ControlTarget != null && !m_Mobile.ControlTarget.Deleted && m_Mobile.ControlTarget != m_Mobile )
			{
				...
				{	
					...
					if ( WalkMobileRange( m_Mobile.ControlTarget, 1, true, 0, 1 ) )
					{
						if ( m_Mobile.Combatant != null && !m_Mobile.Combatant.Deleted && m_Mobile.Combatant.Alive && !m_Mobile.Combatant.IsDeadBondedPet )
						{
							m_Mobile.Warmode = true;
		...
		public virtual bool DoOrderGuard()
		{
			...
			else
			{
				...
				WalkMobileRange( controlMaster, 1, true, 0, 1 );
			}
		...
		public double TransformMoveDelay( double delay )
		{
			...
			{
				delay *= 1.0 + 1.5*( ( m_Mobile.HitsMax - m_Mobile.Hits ) / ((double)m_Mobile.HitsMax ) );

				if ( delay < 0.3 )
					delay = 0.3;
			}

			if ( m_Mobile.Controled && m_Mobile.ControlOrder == OrderType.Follow )
				return delay * 0.5;
			else
				return delay;
		}
		...
		public virtual bool CheckFlee()
		{
			/*if ( m_Mobile.CheckFlee() )
			{
				...
				else
				{
					...
					m_Mobile.Direction = d&Direction.Mask;
					m_Mobile.Move( d );
				}
		...
		public virtual bool MoveTo( Mobile m, bool run, int range )
		{
			...
			if ( m_Mobile.InRange( m, range ) || !m_Mobile.CanSee( m ) )
			{
				m_Path = null;
				return true;
			}
			...
			else if ( !DoMove( m_Mobile.GetDirectionTo( m ) ) )
			{
		...
		public virtual bool AquireFocusMob(int iRange, FightMode acqType, bool bPlayerOnly, bool bFacFriend, bool bFacFoe)
		{
			...
			if ( map != null )
			{
				...
				foreach ( Mobile m in eable )
				{
					...
					if ( m == m_Mobile.BardMaster && m_Mobile.BardProvoked && m_Mobile.BardEndTime > DateTime.Now )
					{
						if ( m_Mobile.BardTarget != null && !m_Mobile.BardTarget.Deleted && m_Mobile.BardTarget.Alive && m_Mobile.BardTarget.InRange( m_Mobile, m_Mobile.RangePerception ) )
							continue;
						else
							m_Mobile.BardProvoked = false;
					}
					...
					/*if ( (bCheckIt && (acqType == FightMode.Agressor || acqType == FightMode.Evil)) || (!bCheckIt && m_Mobile.Summoned && !m_Mobile.Controled) )
					{
						if ( acqType == FightMode.Agressor || acqType == FightMode.Evil )
						{
							...
							if ( acqType == FightMode.Evil && !bCheckIt )
								bCheckIt = Notoriety.Compute( m_Mobile, m ) == Notoriety.Murderer;
						}
					}*/

					bool forcedAggCheck = false;
					if ( acqType == FightMode.Agressor )
						bCheckIt = PlayerMobile.CheckAggressors( m_Mobile, m );
					else if ( acqType == FightMode.Evil )
						bCheckIt = Notoriety.Compute( m_Mobile, m ) == Notoriety.Murderer || PlayerMobile.CheckAggressors( m_Mobile, m );
					else if ( !bCheckIt )
						forcedAggCheck = bCheckIt = PlayerMobile.CheckAggressors( m_Mobile, m );

					if ( bCheckIt && !m.IsDeadBondedPet )
					{
						double theirVal = m_Mobile.GetValueFrom( m, acqType, bPlayerOnly );
						/*
						if we forced an aggressor check, then weigh it much lower than regular checks... 
						this way, aggressors will still be attacked, but only if theres not much else around
						(comes into play with provo, where thigns are aggressed and being attacked, 
						but we dont notice them cause we aren't fightmode aggressor)
						*/
						if ( forcedAggCheck ) 
							theirVal *= ( theirVal > 0 ? 0.5 : 2.0 );
